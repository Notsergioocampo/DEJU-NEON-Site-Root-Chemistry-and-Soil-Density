...
ggplot(summary_data, aes(x = sizeCategory, y = mean_percent, fill = element)) +
  ...
  expand_limits(y = max(summary_data$mean_percent + summary_data$se, na.rm = TRUE) + 5)
...
soil_data <- loadByProduct(dpID = "DP1.00096.001", site = c("DEJU"), check.size = FALSE)
...
p1 <- ggplot(merged_soil_root, aes(x = bulkDensExclCoarseFrag, y = nitrogenPercent)) +
...
p2 <- ggplot(merged_soil_root, aes(x = bulkDensExclCoarseFrag, y = carbonPercent)) +
...
# 🧪 Summary:
# While DEJU lacks direct soil nitrogen or carbon chemistry data, bulk density provides
# indirect insight into soil compaction and porosity, which may influence nutrient availability
# and root nutrient content.
